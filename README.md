...
- **인사말 캐싱**: 인사 음성은 최초 1회만 생성해 디스크에 저장, 이후 실행에서는 재사용 (TTS 크레딧 0)
- **글로벌 단축키**: `⌘⇧Space` — 다른 앱을 쓰다가도 바로 Foxy와 음성 대화 시작/종료
...
- [x] 로그인 시 자동 시작(설정창), 글로벌 단축키 `⌘⇧Space`(어디서든 Foxy와 음성 대화 토글)
